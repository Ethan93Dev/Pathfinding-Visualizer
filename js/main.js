...
function roundToNearestNTH(num) {
  ...
  return num < Math.round(num / NTH) * NTH
    ? Math.round(num / NTH) * NTH - NTH
    : Math.round(num / NTH) * NTH;
}
    ...
    if (!frontier.isEmpty() && isOkayToRun) {
      ...
      for (let i = 0; i < successors.length; i++) {
        ...
        if (
          ...
          (!costSoFar[getNeighbourLocation] || newCost < neighbour.weight)
        ) {
          neighbour.parent = currentNode;
          GRID.paintNode(neighbour.x, neighbour.y, "#9f95ad");
    ...
    if (!frontier.isEmpty() && isOkayToRun) {
      ...
      for (let i = 0; i < successors.length; i++) {
        ...
        if (
          neighbour &&
          neighbour.isTraversable &&
          (!costSoFar[getNeighbourLocation] ||
            newCost < costSoFar[getNeighbourLocation])
        ) {
          neighbour.parent = currentNode;
          GRID.paintNode(neighbour.x, neighbour.y, "#9f95ad");
...
function startPathfindingAlgorithm() {
  // not scalable.
  if (algorithmHasran) {
    algorithmHasran = false;
    isOkayToRun = false;
